...
## Args:        see above
## Returns:     list()
##              dat.s1: data.frame in first dimension with columns c("id", "gridPoints", "Y1", "dt"), for further analysis using MST-FPCA
...
##              dat.s2: data.frame in second dimension with columns c("id", "gridPoints", "Y2", "dt"), for further analysis using MST-FPCA
...
efun11 <- eigen_temp11$vectors[,1:M]

# Estimate univariate PC scores in the first dimension
Yc11  <- t(Xmat1) # Centered data
e.scores11 <- matrix(0, nrow = nregion, ncol = M)
for(i in 1:nregion){
  e.scores11[i,] <- apply(matrix(rep(Yc11[i,], M), nrow = ngrid) * efun11[,1:M],2, mean) 
}
...
efun22 <- eigen_temp22$vectors[,1:M]

# Estimate univariate PC scores in the 2nd dimension
Yc22       <- t(Xmat2) # Centered data
e.scores22 <- matrix(0, nrow = nregion, ncol = M)
for(i in 1:nregion){
  e.scores22[i,] <- apply(matrix(rep(Yc22[i,], M), nrow = ngrid) * efun22[,1:M],2, mean) 
}
...
FPCAout <- MST_FPCA_univariate_decomposition(Y1 = Y1,  # dataframe for 1st dimension nregion * ngrid
                                  Y2 = Y2   # dataframe for 1st dimension nregion * ngrid
)
